...
module.exports = {
    ...
    func: async(msg, { vk }) => { // Параметр асинхронной функции
        ...
        //....ID = пересланные сообщения есть ? (если да) айди равно айди пересланного : (если нет) айди равно текст, разделённый на пробелы (.split(' ')), из которого убрано первое слово (.slice(1)) и который был возвращен обратно в строку (.join(' '))
        ...
        }); // Сам метод получения юзера, выполненный асинхронно (через await).

        // Создаём переменные, содержащие информацию о пользователе
